...
socket.on('output',(data) => { //recieves the entire chat history upon logging in between two users and displays them
    ...
    if(data && data.length > 0){
        document.getElementsByClassName('no_message_dv')[0].style.visibility = 'hidden';
    }else{
        document.getElementsByClassName('no_message_dv')[0].style.visibility = 'visible';
        var container = document.getElementById("chat-messages");
        var elements = container.getElementsByClassName("message");
